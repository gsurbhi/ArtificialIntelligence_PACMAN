...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
       ...
       # util.raiseNotDefined()
        ...
        if noisyDistance == None:
            #update possible position probability to 1 if all ghosts are captured
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        allPossiblePos = util.Counter()
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                allPossiblePos[newPos] += prob * self.beliefs[oldPos]

        self.beliefs = allPossiblePos
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.particles:
                ...
                bPrime = beliefDist[p]
                newBelief[p] = weight * bPrime

            if newBelief.totalCount() == 0:
                self.initializeUniformly(gameState)
            else:
                self.particles = [util.sample(newBelief) for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        #self.particles = [util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))) for oldPos
         #                 in self.particles]
        particleList = []
        for i in range(self.numParticles):
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[i]))
            particleList.append(util.sample(newPosDist))
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] == None:
                    for p in range(self.numParticles):
                        tempList[p] = self.getParticleWithGhostInJail(self.particles[p], i)
